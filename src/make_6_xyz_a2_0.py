...
def get_xyzR_lines(xyzaarray,file_description):
    lines = [     
        '%mem=15GB\n',
        '%nproc=40\n',
    ...
    mol_len = len(xyzaarray)//2
    atom_index = 0
    mol_index = 0
    for x,y,z,a in xyzaarray:
        atom = a
...
def get_one_exe(file_name,machine_type):
    ...
    cc_list=[
        ...
        '\n',
        'export GAUSS_SCRDIR=/home/scr/$JObID \n',
        'mkdir /home/scr/$JObID \n',
        '\n',
        'g16 < {}.inp > {}.log \n'.format(file_basename,file_basename),
        '\n',
        'rm -rf /home/scr/$JObID \n',
        '\n',
        '\n',
        '#sleep 5 \n'
#          '#sleep 500 \n'
...
def make_xyzfile(monomer_name,params_dict):
    a1 = params_dict['a1']; a2 = params_dict['a2']; b = params_dict['b']
    ...
    monomer_array_p1 = get_monomer_xyza(monomer_name,0,b,2*Rt,A2,A3)##1,2がb方向
    monomer_array_p2 = get_monomer_xyza(monomer_name,0,-b,-2*Rt,A2,A3)##1,2がb方向
    monomer_array_p3 = get_monomer_xyza(monomer_name,a1,0,0,A2,A3)##3,4がa方向
    monomer_array_p4 = get_monomer_xyza(monomer_name,-a2,0,0,A2,A3)##3,4がa方向
    monomer_array_t1 = get_monomer_xyza(monomer_name,a1/2,b/2,Rt,A2,-A3)
    monomer_array_t2 = get_monomer_xyza(monomer_name,a1/2,-b/2,-Rt,A2,-A3)
    monomer_array_t3 = get_monomer_xyza(monomer_name,-a2/2,-b/2,-Rt,A2,-A3)
    monomer_array_t4 = get_monomer_xyza(monomer_name,-a2/2,b/2,Rt,A2,-A3)
    xyz_list=['400 \n','polyacene9 \n']##4分子のxyzファイルを作成
...
def make_xyz(monomer_name,params_dict):
    ...
    for key,val in params_dict.items():
        if key in ['a1','a2','b','Rt','cx','cy','cz','theta']:
            val = np.round(val,2)
...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,isInterlayer):
    a1 = params_dict['a1']; a2 = params_dict['a2']; b = params_dict['b']; c = np.array([params_dict.get('cx',0.0),params_dict.get('cy',0.0),params_dict.get('cz',0.0)])
    Rt = params_dict.get('Rt',0.0); A2 = params_dict.get('A2',0.0); A3 = params_dict['theta']
    
    monomer_array_i = get_monomer_xyza(monomer_name,0,0,0,A2,A3)
    monomer_array_p1 = get_monomer_xyza(monomer_name,0,b,0,A2,A3)##p2がa方向
    monomer_array_t1 = get_monomer_xyza(monomer_name,a1/2,b/2,Rt,A2,-A3)
    monomer_array_t2 = get_monomer_xyza(monomer_name,a1/2,-b/2,-Rt,A2,-A3)
    monomer_array_t3 = get_monomer_xyza(monomer_name,-a2/2,-b/2,-Rt,A2,-A3)
    monomer_array_t4 = get_monomer_xyza(monomer_name,-a2/2,b/2,Rt,A2,-A3)
